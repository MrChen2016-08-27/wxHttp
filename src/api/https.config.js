...
// 登录页地址
const loginPage = '/pages/login/main'
...
https.interceptors.request.use(reqUrl => {
    ...
    // 获取 token 放入 headers
    https.defaults.headers.Authorazition = wx.getStorageSync('token') || 'no token'
    ...
    (res, reqUrl) => {
        ...
        switch (res.statusCode) {
            ...
            case 401:
                ...
                reqNumber = 0
                // 如果 wxSession 已经存储接收到 401 代表 redis 中信息已经过期或者未登录
                if (token) {
        ...
        // 判断是否成功，不同规则可自行调整
        if (Number(res.data.result) === 0 || !res.data.result) {
            return res
        } else {
            wx.showModal({
...
function handlerList (reqUrl, data) {
    ...
    if (endStr.indexOf('list') >= 0 && data.data.rows.length <= 0) {
        wx.showToast({
            icon: 'success',
            title: '已加载全部数据'
        })
